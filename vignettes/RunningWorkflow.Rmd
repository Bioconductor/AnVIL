...
```{r inputs-outputs, eval = has_gcloud}
...
```{r change-input, eval = has_gcloud}
inputs <-
    inputs |>
    mutate(
        attribute = ifelse(
...
```{r update-config, eval = has_gcloud}
...
```{r checking-workflow, eval = has_gcloud}
avworkflow_jobs()
```
...
```{r stop-workflow, eval = has_gcloud}
...
```{r workflow-info-example1-retrieval, eval = FALSE}
...
```{r workflow-info-1-read, message=FALSE, warning=FALSE}
info_file_1 <-
...
```{r workflow-info-1-inputs-unnested}
info_1 |>
...
```{r workflow-info-1-outputs-unnested}
info_1 |>
...
```{r workflow-info-example2-retrieval, eval = FALSE, eval = has_gcloud}
...
```{r workflow-info-2-read, message=FALSE, warning=FALSE}
info_file_2 <-
...
```{r workflow-info-2-outputs-unnested, message=FALSE, warning=FALSE}
info_2 |>
    select(workflowId, outputs) |>
    tidyr::unnest_wider(outputs)
```
...
```{r workflow-info-2-output-files}
output_files <-
    info_2 |>
    select(workflowId, outputs) |>
    tidyr::unnest_wider(outputs) |>
...
```{r workflow-info-2-output-paths}
output_files |>
    as.vector()
```
