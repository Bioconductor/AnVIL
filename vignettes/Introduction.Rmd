...
- Define an environment variable or `option()` named `GCLOUD_SDK_PATH`
  pointing to the root of the SDK installation, e.g,
    ...
    ```{r, eval = FALSE}
    dir(Sys.getenv("GCLOUD_SDK_PATH"))
    ##  [1] "bin"                 "completion.bash.inc" "completion.zsh.inc" 
    ##  [4] "data"                "deb"                 "install.bat"        
    ##  [7] "install.sh"          "lib"                 "LICENSE"            
    ## [10] "path.bash.inc"       "path.fish.inc"       "path.zsh.inc"       
    ## [13] "platform"            "properties"          "README"             
    ## [16] "RELEASE_NOTES"       "rpm"                 "VERSION"            
    ```

  Test the installation with `gcloud_exists()`
  
    ```{r, eval = TRUE}
    gcloud_exists() # TRUE when gcloud binaries are available
    ```
