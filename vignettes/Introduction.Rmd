...
- Find the path to the `gcloud` SDK installation, e.g., with
  `Sys.which("gcloud")` in R or either `which gcloud` or
...
```{r install-genomicfeatures, eval = FALSE}
...
```{r gcloud-account-project, eval = has_gcloud}
...
```{r gcloud-projects-list, eval = has_gcloud}
...
```{r gsutil-ls-stat, eval = has_gcloud}
gsutil_ls(src)
...
```{r avworkspace-set-hidden, include = FALSE, cache = FALSE, eval = has_gcloud}
...
```{r avworkspace-get, eval = has_gcloud}
...
```{r avworkspace-set, eval = has_gcloud}
...
```{r avtables-avtable, eval = has_gcloud}
avtables()
...
```{r avtable-manipulation, eval = has_gcloud}
sample |>
...
```{r avtable-import-example, eval = FALSE}
my_cars <-
    mtcars |>
...
```{r avtable-import-status-example, eval = FALSE}
...
```{r avtable-import-pagesize-example, eval = FALSE}
...
```{r avtable-import-set-example, eval = FALSE}
...
```{r write-to-bucket-example, eval = FALSE}
...
```{r drs-uri-example, eval = has_gcloud}
uri <- c(
...
```{r drs-access-url-example, eval = FALSE}
...
```{r terra-status-access, eval = has_gcloud}
terra$status
```

```{r terra-status-call, eval = has_gcloud}
terra$status()
```
...
```{r terra-create-billing-args, eval = has_gcloud}
...
```{r terra-overwrite-args, eval = has_gcloud}
...
```{r terra-status-response, eval = has_gcloud}
status <- terra$status()
class(status)
```
...
```{r terra-status-str, eval = has_gcloud}
str(status)
```
...
```{r terra-status-flatten-example, eval = has_gcloud}
lst <- status |> as.list()
lengths(lst)
lengths(lst$systems)
str(lst$systems)
