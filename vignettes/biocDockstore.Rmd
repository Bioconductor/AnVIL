...
used to process API requests.  We'll define a little
bit of temporary infrastructure to show what 'dockstore'
can do.

```{r getmeths}
api = function(x) slot(x, "api")
sapply(api(dockstore)$tags, "[[", "name")
```

These are groups of API components.  We're interested in
the 'users' component.  Higher level methods will be
introduced to help here, but for now we stick to base R
methods.

```{r lku}
umeth = sort(grep("[uU]ser", names(operations(dockstore)), value=TRUE))
decode = function(x) rjson::fromJSON(httr::content(x, "text"))
umeth
```
